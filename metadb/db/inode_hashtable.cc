...
int InodeHashTable::HashEntryInsetKV(InodeHashVersion *version, uint32_t index, const inode_id_t key, const pointer_t value){
    NvmInodeHashEntry *entry = &(version->buckets_[index]);
    InodeHashEntryLinkOp op;
    op.root = entry->root;
    op.res = entry->root;
    int res = InodeHashEntryLinkInsert(op, key, value);
...
int HashEntryUpdateKV(InodeHashVersion *version, uint32_t index, const inode_id_t key, const pointer_t value){
    ...
    op.root = entry->root;
    op.res = entry->root;
    int res = InodeHashEntryLinkUpdate(op, key, value);
    HashEntryDealWithOp(version, index, op);
    return res;
}
...
int HashEntryDeleteKV(InodeHashVersion *version, uint32_t index, const inode_id_t key){
    ...
    op.root = entry->root;
    op.res = entry->root;
    int res = InodeHashEntryLinkDelete(op, key);
    HashEntryDealWithOp(version, index, op);
    return res;
}
...
int InodeHashTable::Put(const inode_id_t key, const pointer_t value){
    ...
    int res = HashEntryInsetKV(version, index, key, value);
...
int InodeHashTable::Update(const inode_id_t key, const pointer_t value){
    bool is_rehash = false;
    InodeHashVersion *version;
    GetVersionAndRefByWrite(is_rehash, &version);
    uint32_t index = hash_id(key, version->capacity_);
...
int InodeHashTable::Delete(const inode_id_t key){
