...
void log_msg(const char *format, ...)
{
    va_list ap;
    va_start(ap, format);
    vfprintf(GENERAL_DATA->logfile, format, ap);
}
...
void log_fuse_context(struct fuse_context *context)
{
    ...
    log_struct(((struct general_state *)context->private_data), logfile, %08x, );
    log_struct(((struct general_state *)context->private_data), rootdir, %s, );
